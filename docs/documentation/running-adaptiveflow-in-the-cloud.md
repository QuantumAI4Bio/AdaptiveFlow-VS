...
To run AdaptiveFlow on Amazon Web Services, we recommend to use AWS ParallelCluster, which allows to employ to run the SGE, TORQE, and Slurm resource managers conventiently in on AWS. All three of them are supported by AdaptiveFlow  2.0, but we recommend to use Slurm, as it is most common and modern, and best supported resource manager. We recommend the following page to get started with AWS ParallelCluster.
