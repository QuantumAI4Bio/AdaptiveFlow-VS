...
In addition to the general preparation which is required for any AdaptiveFlow module as describe [here](../../documentation/installation/), for AFVS the `input-files` folder needs to be set up as described [below](preparing-the-workflow.md#preparation-of-the-input-files-folder). The exemplified step-by-step preparation can be found in the [Tutorials](broken-reference).
...
There, specific subsets of the database can be downloaded, as well as the required collection lengths file which is needed to set up the `tools` folder as described earlier [here](../../documentation/using-adaptiveflow/preparing-the-workflow.md#central-task-list) (i.e. this file can be used for the central task `todo.all` list which is used by AdaptiveFlow).
