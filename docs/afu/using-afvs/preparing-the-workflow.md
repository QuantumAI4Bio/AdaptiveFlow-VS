...
In addition to the general preparation which is required for any AdaptiveFlow module as describe [here](broken-reference), for AFVS the `input-files` folder needs to be set up as described [below](broken-reference). The exemplified step-by-step preparation can be found in the [Tutorials](broken-reference).

## Preparation of the `input-files` Folder

For using AFVS, a set of docking scenarios has to be set up by the user, and it can be any number of them. This is done in the `input-files` folder, which contains the following items:

* Configuration files for the docking programs
* Receptor structures
* Other files needed by the docking program
* Optional: Input ligand collection in ready-to-dock AdaptiveFlow format (can also be located at another location, which can be specified in the control file)
...
* the ligand, since the ligand is variable and AFVS takes care of it

The number of CPU cores (multi threading) in the config file, which can be specified via the option `cpu`,  will be replaced by the option in the AdaptiveFlow control file option `cpus_per_queue`. Still, the `cpu` option needs to be present in the docking program config file, so that AdaptiveFlow can replace the value by the value of `cpus_per_queue`.&#x20;



PLANTS

As always in AdaptiveFlow, also the config file of the docking program PLANTS is placed in a subfolder in the `input-files` directory, and needs to contain a file called `config.txt`. All file paths which are set in the `config.txt` file need to be relative to the tools folder, as for any other docking program.
...
PLANTS requires ligands to be in the MOL2 format. AdaptiveFlow can therefore use libraries in the typical AdaptiveFlow  library format where all ligands are stored in the MOL2 format. Alternatively,  AdaptiveFlow Ants can also use ligand libraries in the PDBQT Format, and convert them on the fly to the MOL2 format so that PLANTS can use PDBQT libraries. This library format option can be set in the control file via the parameter `ligand_ibrary_format`. If the library is in the PDBQT format, then SPORES has to be installed for the on the fly conversion. More details on how to install SPORES and PLANTS can be found here:

[https://docs.adaptiveflow.org/documentation/-LdE8RH9UN4HKpckqkX3/afvs/installation/prerequisites](https://docs.adaptiveflow.org/documentation/-LdE8RH9UN4HKpckqkX3/afvs/installation/prerequisites)
...
The required ligand input database can be prepared either from scratch with AFLP, or a ready-made database can be used. For virtual screenings, most users would use a ready-made database. For this purpose, we are providing the REAL database from Enamine, which can be downloaded here from the AdaptiveFlow website:&#x20;

* [https://adaptiveflow.org/real-library](https://adaptiveflow.org/real-library)

There, specific subsets of the database can be downloaded, as well as the required collection lengths file which is needed to set up the `tools` folder as described earlier [here](broken-reference) (i.e. this file can be used for the central task `todo.all` list which is used by AdaptiveFlow).
