...
wget https://virtual-flow.org/sites/virtual-flow.org/files/tutorials/VFVS\_GK.tar
...
\
From the resulting folder, copy the following files to the VFVS folder: \
\
`cp -v -r ./VFVS_GK/input-files/qvina02_rigid_receptor1/ VFVS_GK/input-files/receptor/ ~/VFVS/input-files/`\
\
...
Next, we should copy the _Enamine\_REAL\_Space\_2022q12\_sparse.todo_ file to the VFVS folder, overwriting the todo.all file, using a command similar to the one below:\
\
...
and copy it to overwrite the file in the VFVS folder,  using a command similar to the one below:\
\
...
`./vfvs_prepare_folders.py` (with --overwrite if you retry) \
\
followed by: \
\
