...
In order to set up the workflow, go to the folder `VFVS/tools` This is the working directory of VirtualFlow, where all the commands to run the software are started. Enter the following command to go to this folder:
...
All other files required to start the VirtualFlow Workflow are already set up. Further information on these other preconfigured files can be found in the [documentation](https://app.gitbook.com/o/-LNuBCz49KX3ym-BKSfI/s/-LNuBCz54OweSMdPmK2T/).&#x20;
...
It can happen that this command requires to be run as root, depending on your specific setup. Please try `sudo ./vfvs_build_docker.sh` in case the above command fails.&#x20;
