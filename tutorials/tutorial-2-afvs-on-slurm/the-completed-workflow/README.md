...
## To view the results of the completed workflow, go to the folder labeled `output-files` located within the AFVS directory:
...
To obtain the full ligand ranking, the AFTools package can be used, which can be installed as described [here](https://docs.adaptiveflow.org/documentation/-LdE8RH9UN4HKpckqkX3/aftools/installation-1). Assuming it is installed, we create a new folder for the post-processing of the data. In the AFVS root directory we create a new folder:
...
After that, we use the command `afvs_pp_docking_all.sh` an:

```
afvs_pp_ranking_all.sh ../../output-files/complete/ 2 meta_collection
```
...
For long-term storage, we create a tar archive of either the entire AdaptiveFlow root folder:

```
cd ../../..
tar -xvzf AFVS_GK.tar.gz AFVS_GK
```

## Uninstallation

To uninstall AFVS, one simply needs to delete the root folder of the workflow:
